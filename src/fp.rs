//! This module contains functions for filling the stark trace and adding constraints for the corresponding trace for some Fp operations (multiplication, addition, subtraction, etc). One fp element is represented as \[u32; 8\] inside the trace.
...
use crate::{native::{add_u32_slices, add_u32_slices_8, get_bits_as_array, get_div_rem_modulus_from_biguint_8, get_selector_bits_from_u32, get_u32_vec_from_literal, get_u32_vec_from_literal_16, modulus, mul_u32_slice_8, multiply_by_slice, sub_u32_slices, sub_u32_slices_8, Fp}, utils::*};

// Fp Multiplication layout offsets
/*
    These trace offsets are for long multiplication. The inputs are each of 8 limbs. The trace needs 8 rows.
    to compute the result of the multiplication. The final result is stored in the slice [SUM_OFFSET..SUM_OFFSET+16].
    X_INPUT_OFFSET -> offset at which the first input is set.
    Y_INPUT_OFFSET -> offset at which the second input is set.
    XY_OFFSET -> offset of x * y[i] where 0 <= i < 8.
    ...
    SHIFTED_XY_OFFSET -> offset at which the shifted values of x * y\[i\] are set. In long multiplication, the multiplication of the i-th digit
        is set after shifting the result by i places. This is exactly that shift. The maximum shift can be 7, hence the maximum result can be of
        length 16. Therefore, 16 places are reserved for this field.
    SELECTOR_OFFSET -> offset specifying which index of y are we using for multiplication in the current row. Total 8 selectors, one for each limb.
...
pub const Y_INPUT_OFFSET: usize = X_INPUT_OFFSET + 8;
pub const XY_OFFSET: usize = Y_INPUT_OFFSET + 8;
pub const XY_CARRIES_OFFSET: usize = XY_OFFSET + 9;
pub const SHIFTED_XY_OFFSET: usize = XY_CARRIES_OFFSET + 8;
pub const SELECTOR_OFFSET: usize = SHIFTED_XY_OFFSET + 16;
pub const SUM_OFFSET: usize = SELECTOR_OFFSET + 8;
pub const SUM_CARRIES_OFFSET: usize = SUM_OFFSET + 16;
pub const MULTIPLICATION_SELECTOR_OFFSET: usize = SUM_CARRIES_OFFSET + 16;
...
/*
    These trace offsets are for long addition. The inputs are 16 limbs each. The trace needs 1 row to compute the result.
...
pub const ADDITION_Y_OFFSET: usize = ADDITION_X_OFFSET + 16;
pub const ADDITION_SUM_OFFSET: usize = ADDITION_Y_OFFSET + 16;
pub const ADDITION_CARRY_OFFSET: usize = ADDITION_SUM_OFFSET + 16;
pub const ADDITION_TOTAL: usize = ADDITION_CARRY_OFFSET + 16;

// Non reduced subtraction layout offsets
/*
    These trace offsets are for long subtraction. The inputs are 16 limbs each. The trace needs 1 row to compute the result. Assume x > y.
...
pub const SUBTRACTION_Y_OFFSET: usize = SUBTRACTION_X_OFFSET + 16;
pub const SUBTRACTION_DIFF_OFFSET: usize = SUBTRACTION_Y_OFFSET + 16;
pub const SUBTRACTION_BORROW_OFFSET: usize = SUBTRACTION_DIFF_OFFSET + 16;
pub const SUBTRACTION_TOTAL: usize = SUBTRACTION_BORROW_OFFSET + 16;

// Reduce and rangecheck layout offsets
/*
    These trace offsets are for reducing a [u32; 16] input with the bn254 field prime. Ensures, x = d*p + r. Where x is the input,
    d is the quotient, p is the prime and r is the reduced output. The trace needs 8 rows.
...
pub const REDUCTION_ADDITION_OFFSET: usize = REDUCE_X_OFFSET + 16;
pub const REDUCED_OFFSET: usize = REDUCTION_ADDITION_OFFSET + ADDITION_TOTAL;
pub const REDUCTION_TOTAL: usize = REDUCED_OFFSET + 8;

// Rangecheck offsets
// whenever range check is used, start_col - 8 will contain the element being rangechecked
/*
    These trace offsets are for checking if a given input is less than the bn254 field prime. Needs 1 row for the computation. The check works as follows ->
        1. Compute y = (2**255 - p + x)
        2. If (y>>255)&1 == 0, then x in less than p.
...
pub const RANGE_CHECK_SUM_CARRY_OFFSET: usize = RANGE_CHECK_SUM_OFFSET + 8;
pub const RANGE_CHECK_BIT_DECOMP_OFFSET: usize = RANGE_CHECK_SUM_CARRY_OFFSET + 8;
pub const RANGE_CHECK_TOTAL: usize = RANGE_CHECK_BIT_DECOMP_OFFSET + 32;

// Fp addition layout offsets
/*
    These trace offsets are for long addition. The inputs are 8 limbs each. The trace needs 1 row to compute the result.
...
pub const FP_ADDITION_Y_OFFSET: usize = FP_ADDITION_X_OFFSET + 8;
pub const FP_ADDITION_SUM_OFFSET: usize = FP_ADDITION_Y_OFFSET + 8;
pub const FP_ADDITION_CARRY_OFFSET: usize = FP_ADDITION_SUM_OFFSET + 8;
pub const FP_ADDITION_TOTAL: usize = FP_ADDITION_CARRY_OFFSET + 8;

// Fp subtraction layout offsets
/*
    These trace offsets are for long subtraction. The inputs are 8 limbs each. The trace needs 1 row to compute the result. Assume x > y.
...
pub const FP_SUBTRACTION_Y_OFFSET: usize = FP_SUBTRACTION_X_OFFSET + 8;
pub const FP_SUBTRACTION_DIFF_OFFSET: usize = FP_SUBTRACTION_Y_OFFSET + 8;
pub const FP_SUBTRACTION_BORROW_OFFSET: usize = FP_SUBTRACTION_DIFF_OFFSET + 8;
pub const FP_SUBTRACTION_TOTAL: usize = FP_SUBTRACTION_BORROW_OFFSET + 8;

// Fp multiply single
/*
    These trace offsets are for long multiplication. The first input is 8 limbs, the second input is 1 limb. The trace needs 1 row to compute the result.
    ...
    FP_MULTIPLY_SINGLE_Y_OFFSET -> offset at which second input set.
...
pub const FP_MULTIPLY_SINGLE_Y_OFFSET: usize = FP_MULTIPLY_SINGLE_X_OFFSET + 8;
pub const FP_MULTIPLY_SINGLE_SUM_OFFSET: usize = FP_MULTIPLY_SINGLE_Y_OFFSET + 1;
pub const FP_MULTIPLY_SINGLE_CARRY_OFFSET: usize = FP_MULTIPLY_SINGLE_SUM_OFFSET + 8;
pub const FP_MULTIPLY_SINGLE_TOTAL: usize = FP_MULTIPLY_SINGLE_CARRY_OFFSET + 8;

// Fp reduce rangecheck single
/*
    These trace offsets are for for reducing a [u32; 8] input with the bn254 field prime. Ensures, x = d*p + r. Where x is the input,
...
pub const FP_SINGLE_REDUCTION_ADDITION_OFFSET: usize = FP_SINGLE_REDUCE_X_OFFSET + 8;
pub const FP_SINGLE_REDUCED_OFFSET: usize = FP_SINGLE_REDUCTION_ADDITION_OFFSET + FP_ADDITION_TOTAL;
pub const FP_SINGLE_REDUCE_TOTAL: usize = FP_SINGLE_REDUCED_OFFSET + 8;
...
/// Fills the stark trace of addition following long addition. Inputs are 16 limbs each. Needs 1 row.
...
/// Fills the stark trace of addition following long addition. Inputs are 8 limbs each. Needs 1 row.
...
>(
    ...
    let (x_y_sum, x_y_sum_carry) = add_u32_slices_8(&x, &y);
...
/// Fills the stark trace of negation. Input is 8 limbs. Needs 1 row. In essence, it fills an addition trace with inputs as `x` and `-x`.
...
/// Fills the stark trace of subtraction following long subtraction. Inputs are 16 limbs each. Needs 1 row. Assume x > y.
...
/// Fills the stark trace of subtraction following long subtraction. Inputs are 8 limbs each. Needs 1 row. Assume x > y.
...
>(
    ...
    let (x_y_diff, x_y_borrow) = sub_u32_slices_8(&x, &y);
    ...
    assign_u32_in_series(trace, row, start_col + FP_SUBTRACTION_DIFF_OFFSET, &x_y_diff);
    assign_u32_in_series(trace, row, start_col + FP_SUBTRACTION_BORROW_OFFSET, &x_y_borrow);
}

/// Fills the stark trace of multiplication following long multiplication. Inputs are 8 limbs and 1 limb respectively. Needs 1 row.
...
>(
    ...
    let (x_y_sum, x_y_carry) = mul_u32_slice_8(x, y);
    ...
    assign_u32_in_series(trace, row, start_col + FP_MULTIPLY_SINGLE_SUM_OFFSET, &x_y_sum);
    assign_u32_in_series(trace, row, start_col + FP_MULTIPLY_SINGLE_CARRY_OFFSET, &x_y_carry);
}

/// Fills the stark trace of reducing wrt modulo p. Input is 8 limbs. Needs 1 row. Returns the answer as \[u32; 8\].
...
>(
    ...
    let (div, rem) = get_div_rem_modulus_from_biguint_8(BigUint::new(x.to_vec()));
    let div = div[0];
    let modulus = get_u32_vec_from_literal(modulus());
    fill_trace_multiply_single_fp(trace, &modulus, div, row, start_col + FP_SINGLE_REDUCE_MULTIPLICATION_OFFSET);
    ...
    fill_trace_addition_fp(trace, &div_x_mod, &rem, row, start_col + FP_SINGLE_REDUCTION_ADDITION_OFFSET);
    rem
}

/// Fills the stark trace for range check operation wrt the field prime p. Input is 8 limbs. Needs 1 row.
pub fn fill_range_check_trace<F: RichField + Extendable<D>,
    const D: usize,
    const C: usize,
>(trace: &mut Vec<[F; C]>, x: &[u32; 8], row: usize, start_col: usize) {
    let y = (BigUint::from(1u32) << 255) - modulus();
    let y_u32 = get_u32_vec_from_literal(y);
    let (x_y_sum, x_y_carry) = add_u32_slices_8(&x, &y_u32);
    ...
    assign_u32_in_series(trace, row, start_col + RANGE_CHECK_SUM_CARRY_OFFSET, &x_y_carry);
    assign_u32_in_series(trace,
        row,
        start_col + RANGE_CHECK_BIT_DECOMP_OFFSET,
        &get_bits_as_array(x_y_sum[7]),
    );
}

/// Fills stark trace for multiplication following long multiplication. Inputs are 8 limbs each. Needs 8 rows.
...
>(
    ...
    // We have calcualted multiplying two max bn254 Fp numbers
    // dont exceed [u32; 16] so no need of [u32; 17]
    let mut prev_xy_sum = [0u32; 16];

    for i in 0..8 {
        let (xy, xy_carry) = multiply_by_slice(&x, y[i]);
        assign_u32_in_series(trace, start_row + i, start_col + XY_OFFSET, &xy);
        assign_u32_in_series(trace, start_row + i, start_col + XY_CARRIES_OFFSET, &xy_carry);

        // fill shifted XY's
        // XY's will have 0-11 number of shifts in their respective rows
        let mut xy_shifted = [0u32; 16];
        for j in 0..9 {
            let shift = i;
            xy_shifted[j + shift] = xy[j];
        }
        assign_u32_in_series(trace, start_row + i, start_col + SHIFTED_XY_OFFSET, &xy_shifted);
        ...
        assign_u32_in_series(trace, start_row + i, start_col + SUM_CARRIES_OFFSET, &xy_sum_carry);

        prev_xy_sum = xy_sum;
    }
}

/// Fills the stark trace of reducing wrt modulo p. Input is 16 limbs. Needs 8 rows. Set addition selector to 1 only in the 7th row, because that's where multiplication result is set. Returns the answer as \[u32; 8\].
...
>(
    ...
    let div_x_mod = get_u32_vec_from_literal_16(
    ...
    let mut rem_16 = [0u32; 16];
    rem_16[0..8].copy_from_slice(&rem);

    fill_addition_trace(trace,
        &div_x_mod,
        &rem_16,
        start_row + 7,
...
/// Constraints the operation for multiplication of two \[u32; 8\].
///
/// Constraint the input values across this row and next row wherever selector is on.
///
/// Constraints the following -> `selector[i] * (product[j] + carries[j]*(2**32) - x[j] * y[i] - carries[j-1]) == 0`. for 0 <= j < 8, for 0 <= i < 8.
/// which encapsulates the condition "either selector is off or the multiplication is correct".
///
/// Constraints the shifted value with product of the current limb as `selector[i] * (shifted[i + j] - product[j]) == 0`. for 0 <= j < 8, for 0 <= i < 8.
...
///
/// Constraints `next_row_sum[i] + next_row_carries[i]*(2**32) == curr_row_sum[i] + shifted[i] + next_row_carries[i-1]` for 0 <= i < 16.
    ...
    P: PackedField<Scalar = FE>,
{
    ...
    yield_constr.constraint_transition(
        bit_selector_val *
        local_values[start_col + MULTIPLICATION_SELECTOR_OFFSET]
            * (local_values[start_col + XY_OFFSET + 8]
                - local_values[start_col + XY_CARRIES_OFFSET + 7]),
    );
...
pub fn add_multiplication_constraints_ext_circuit<
    F: RichField + Extendable<D>,
    const D: usize,
>(
    ...
    for i in 0..12 {
        let tmp = builder.mul_extension(bit_selector_val, local_values[start_col + MULTIPLICATION_SELECTOR_OFFSET]);
    ...
    for i in 0..12 {
        for j in 0..12 {
            if j == 0 {
                let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + SELECTOR_OFFSET + i]);
                let mul_tmp2 = builder.mul_extension(local_values[start_col + X_INPUT_OFFSET + j], local_values[start_col + Y_INPUT_OFFSET + i]);
                let mul_tmp3 = builder.mul_extension(local_values[start_col + XY_CARRIES_OFFSET + j],constant);
    ...
    let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + MULTIPLICATION_SELECTOR_OFFSET]);
    let sub_tmp1 = builder.sub_extension(local_values[start_col + XY_OFFSET + 12], local_values[start_col + XY_CARRIES_OFFSET + 11]);
    ...
    for i in 0..12 {
        for j in 0..13 {
            let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + SELECTOR_OFFSET + i]);
            let sub_tmp1 = builder.sub_extension(local_values[start_col + SHIFTED_XY_OFFSET + j + i], local_values[start_col + XY_OFFSET + j]);
    ...
    for j in 0..24 {
        let mul_tmp1 = builder.mul_extension(bit_selector_val, local_values[start_col + MULTIPLICATION_FIRST_ROW_OFFSET]);
    ...
    for j in 1..24 {
        ...
        let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + MULTIPLICATION_SELECTOR_OFFSET]);
        let mul_tmp2 = builder.mul_extension(next_values[start_col + SUM_CARRIES_OFFSET + j], constant);

        let sub_tmp1 = builder.sub_extension(mul_tmp2,next_values[start_col + SHIFTED_XY_OFFSET + j]);
        let sub_tmp2 = builder.sub_extension(sub_tmp1,local_values[start_col + SUM_OFFSET + j]);
        let sub_tmp3 = builder.sub_extension(sub_tmp2,next_values[start_col + SUM_CARRIES_OFFSET + j - 1]);
...
/// Constraints the addition for addition of two \[u32; 16\].
...
pub fn add_addition_constraints_ext_circuit<
    F: RichField + Extendable<D>,
    const D: usize,
>(
    builder: &mut CircuitBuilder<F, D>,
    ...
    for j in 0..24 {
        if j == 0 {
            let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + ADDITION_CHECK_OFFSET]);
            let mul_tmp2 = builder.mul_extension(local_values[start_col + ADDITION_CARRY_OFFSET + j], constant);

            let sub_tmp1 = builder.sub_extension(mul_tmp2, local_values[start_col + ADDITION_X_OFFSET + j]);
            let sub_tmp2 = builder.sub_extension(sub_tmp1, local_values[start_col + ADDITION_Y_OFFSET + j]);

            let add_tmp1 = builder.add_extension(sub_tmp2, local_values[start_col + ADDITION_SUM_OFFSET + j]);
            ...
            let sub_tmp1 = builder.sub_extension(mul_tmp2, local_values[start_col + ADDITION_X_OFFSET + j]);
            let sub_tmp2 = builder.sub_extension(sub_tmp1, local_values[start_col + ADDITION_Y_OFFSET + j]);
            let sub_tmp3 = builder.sub_extension(sub_tmp2,local_values[start_col + ADDITION_CARRY_OFFSET + j - 1]);


            let add_tmp1 = builder.add_extension(sub_tmp3, local_values[start_col + ADDITION_SUM_OFFSET + j]);
...
/// Constraints the operation for addition of two \[u32; 8\].
/// Constraints the following for every limb -> `sum[i] + carries[i]*(2**32) == x[i] + y[i] + carries[i-1]`.
...
pub fn add_addition_fp_constraints_ext_circuit<
    F: RichField + Extendable<D>,
    const D: usize,
>(
    builder: &mut CircuitBuilder<F, D>,
    ...
    for j in 0..12 {
        if j == 0 {
            let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + FP_ADDITION_CHECK_OFFSET]);
            let mul_tmp2 = builder.mul_extension(local_values[start_col + FP_ADDITION_CARRY_OFFSET + j],constant);

            let sub_tmp1 = builder.sub_extension(mul_tmp2, local_values[start_col + FP_ADDITION_X_OFFSET + j]);
            let sub_tmp2 = builder.sub_extension(sub_tmp1,local_values[start_col + FP_ADDITION_Y_OFFSET + j]);
            ...
            let sub_tmp1 = builder.sub_extension(mul_tmp2, local_values[start_col + FP_ADDITION_X_OFFSET + j]);
            let sub_tmp2 = builder.sub_extension(sub_tmp1,local_values[start_col + FP_ADDITION_Y_OFFSET + j]);
            let sub_tmp3 = builder.sub_extension(sub_tmp2,local_values[start_col + FP_ADDITION_CARRY_OFFSET + j - 1]);
...
/// Constraints the operation for subtraction of two \[u32; 8\].
...
>(
    ...
    for j in 0..12 {
        if j == 0 {
            let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + FP_SUBTRACTION_CHECK_OFFSET]);
...
/// Constraints the negation operation for \[u32; 8\].
...
pub fn add_negate_fp_constraints_ext_circuit<
    F: RichField + Extendable<D>,
    const D: usize,
>(
    builder: &mut CircuitBuilder<F, D>,
    ...
    add_addition_constraints_ext_circuit(builder, yield_constr, local_values, start_col, bit_selector);
    let bit_selector_val = bit_selector.unwrap_or(builder.constant_extension(F::Extension::ONE));
    ...
    for i in 0..12 {
        ...
        let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + FP_ADDITION_CHECK_OFFSET]);
        let sub_tmp1 = builder.sub_extension(local_values[start_col + FP_ADDITION_SUM_OFFSET + i], constant);
...
/// Constraints the operation for multiplication of \[u32; 8\] with a u32.
...
>(
    ...
    for j in 0..12 {
        if j == 0 {
            let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + FP_MULTIPLY_SINGLE_CHECK_OFFSET]);
...
/// Constraints the reduction operation for \[u32; 8\].
...
>(
    ...
    for i in 0..12 {
        ...
        let mul_tmp1 = builder.mul_extension(bit_selector_val, local_values[start_col + FP_SINGLE_REDUCE_MULTIPLICATION_OFFSET + FP_MULTIPLY_SINGLE_CHECK_OFFSET]);
        let sub_tmp1 = builder.sub_extension(local_values[start_col + FP_SINGLE_REDUCE_MULTIPLICATION_OFFSET + FP_MULTIPLY_SINGLE_X_OFFSET + i] , lc);

        let c = builder.mul_extension(mul_tmp1, sub_tmp1);
        yield_constr.constraint_transition(builder, c);
    }

    add_fp_single_multiply_constraints_ext_circuit(builder, yield_constr, local_values, start_col+FP_SINGLE_REDUCE_MULTIPLICATION_OFFSET, bit_selector);

    for i in 0..12 {
        let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + FP_SINGLE_REDUCTION_ADDITION_OFFSET + FP_ADDITION_CHECK_OFFSET]);
        let sub_tmp1 = builder.sub_extension(local_values[start_col + FP_SINGLE_REDUCE_MULTIPLICATION_OFFSET + FP_MULTIPLY_SINGLE_SUM_OFFSET + i] , local_values[start_col + FP_SINGLE_REDUCTION_ADDITION_OFFSET + FP_ADDITION_X_OFFSET + i]);

        let c = builder.mul_extension(mul_tmp1, sub_tmp1);
        yield_constr.constraint_transition(builder, c);
    }

    add_addition_fp_constraints_ext_circuit(builder, yield_constr, local_values, start_col + FP_SINGLE_REDUCTION_ADDITION_OFFSET, bit_selector);

    for i in 0..12 {
        let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + FP_SINGLE_REDUCTION_ADDITION_OFFSET + FP_ADDITION_CHECK_OFFSET]);
        let sub_tmp1 = builder.sub_extension(local_values[start_col + FP_SINGLE_REDUCED_OFFSET + i], local_values[start_col + FP_SINGLE_REDUCTION_ADDITION_OFFSET + FP_ADDITION_Y_OFFSET + i]);
    ...
    for i in 0..12 {
        let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + FP_SINGLE_REDUCTION_ADDITION_OFFSET + FP_ADDITION_CHECK_OFFSET]);
        let sub_tmp1 = builder.sub_extension(local_values[start_col + FP_SINGLE_REDUCE_X_OFFSET + i], local_values[start_col + FP_SINGLE_REDUCTION_ADDITION_OFFSET + FP_ADDITION_SUM_OFFSET + i]);
...
/// Constraints the operation for subtraction of two \[u32; 16\].
/// Constraints the following for every limb -> `diff[i] - borrows[i]*(2**32) == x[i] - y[i] - borrows[i-1]`.
...
pub fn add_subtraction_constraints_ext_circuit<
    F: RichField + Extendable<D>,
    const D: usize,
>(
    builder: &mut CircuitBuilder<F, D>,
    ...
    for j in 0..24 {
        if j == 0 {
            let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + SUBTRACTION_CHECK_OFFSET]);
            let mul_tmp2 = builder.mul_extension(local_values[start_col + SUBTRACTION_BORROW_OFFSET + j], constant);

            let add_tmp1 = builder.add_extension(local_values[start_col + SUBTRACTION_DIFF_OFFSET + j], local_values[start_col + SUBTRACTION_Y_OFFSET + j]);

            let sub_tmp1 = builder.sub_extension(add_tmp1, mul_tmp2);
            let sub_tmp2 = builder.sub_extension(sub_tmp1, local_values[start_col + SUBTRACTION_X_OFFSET + j]);
            ...
            let add_tmp1 = builder.add_extension(local_values[start_col + SUBTRACTION_DIFF_OFFSET + j], local_values[start_col + SUBTRACTION_Y_OFFSET + j]);
            let add_tmp2 = builder.add_extension(add_tmp1, local_values[start_col + SUBTRACTION_BORROW_OFFSET + j - 1]);
...
/// Constraints the range check operation of a \[u32; 8\].
/// Constraints the addition of the input and (2**255)-p. Then constraints the bit decomposition of the most significant limb of the result of the previous addition. Then constraints the 31st bit of the decomposition (which is overall 255th bit of the result) to zero.
    ...
    P: PackedField<Scalar = FE>,
{
    ...
    for i in 0..8 {
        ...
        yield_constr.constraint(
            bit_selector_val * local_values[start_col + RANGE_CHECK_SELECTOR_OFFSET] * (val_reconstructed - local_values[start_col + RANGE_CHECK_SUM_OFFSET + 7])
        );
        yield_constr.constraint(bit_selector_val * local_values[start_col + RANGE_CHECK_SELECTOR_OFFSET] * local_values[bit_col + 31]);
    }
}

pub fn add_range_check_constraints_ext_circuit<
    F: RichField + Extendable<D>,
    const D: usize,
>(
    builder: &mut CircuitBuilder<F, D>,
    ...
    for i in 0..12 {
        ...
        if i == 0 {
            ...
            let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + RANGE_CHECK_SELECTOR_OFFSET]);
            let mul_tmp2 = builder.mul_extension(local_values[start_col + RANGE_CHECK_SUM_CARRY_OFFSET + i], constant);

            let sub_tmp1 = builder.sub_extension(mul_tmp2, lc);
            let sub_tmp2 = builder.sub_extension(sub_tmp1, local_values[start_col - 12 + i]);

            let add_tmp1 = builder.add_extension(sub_tmp2, local_values[start_col + RANGE_CHECK_SUM_OFFSET + i]);
            ...
            let sub_tmp1 = builder.sub_extension(mul_tmp2, lc);
            let sub_tmp2 = builder.sub_extension(sub_tmp1, local_values[start_col - 12 + i]);
            let sub_tmp3 = builder.sub_extension(sub_tmp2, local_values[start_col + RANGE_CHECK_SUM_CARRY_OFFSET + i - 1]);

            let add_tmp1 = builder.add_extension(sub_tmp3, local_values[start_col + RANGE_CHECK_SUM_OFFSET + i]);
...
/// Constraints the reduction operation for \[u32; 16\].
...
pub fn add_reduce_constraints_ext_circuit<
    F: RichField + Extendable<D>,
    const D: usize,
>(
    builder: &mut CircuitBuilder<F, D>,
    ...
    for i in 0..12 {
        ...
        let sub_tmp1 = builder.sub_extension(local_values[start_col + REDUCE_MULTIPLICATION_OFFSET + Y_INPUT_OFFSET + i], lc);
    ...
    add_multiplication_constraints_ext_circuit(builder, yield_constr, local_values, next_values, start_col + REDUCE_MULTIPLICATION_OFFSET, bit_selector);
    for i in 0..24 {
        let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[selector_col]);
        let sub_tmp1 = builder.sub_extension(local_values[start_col + REDUCE_X_OFFSET + i], next_values[start_col + REDUCE_X_OFFSET + i]);
    ...
    for i in 0..12 {
        let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[selector_col]);
        let sub_tmp1 = builder.sub_extension(local_values[start_col + REDUCED_OFFSET + i], next_values[start_col + REDUCED_OFFSET + i]);
    ...
    for i in 0..24 {
        let mul_tmp1 = builder.mul_extension(bit_selector_val, local_values[start_col + REDUCTION_ADDITION_OFFSET + ADDITION_CHECK_OFFSET]);
        let sub_tmp1 = builder.sub_extension(local_values[start_col + REDUCE_MULTIPLICATION_OFFSET + SUM_OFFSET + i], local_values[start_col + REDUCTION_ADDITION_OFFSET + ADDITION_X_OFFSET + i]);

        let c = builder.mul_extension(mul_tmp1, sub_tmp1);
        yield_constr.constraint_transition(builder, c);
    }

    add_addition_constraints_ext_circuit(builder, yield_constr, local_values, start_col + REDUCTION_ADDITION_OFFSET, bit_selector);

    for i in 0..24 {
        if i < 12 {
            let mul_tmp1 = builder.mul_extension(bit_selector_val , local_values[start_col + REDUCTION_ADDITION_OFFSET + ADDITION_CHECK_OFFSET]);
            let sub_tmp1 = builder.sub_extension(local_values[start_col + REDUCED_OFFSET + i], local_values[start_col + REDUCTION_ADDITION_OFFSET + ADDITION_Y_OFFSET + i]);
            ...
            let c = builder.mul_extension(mul_tmp1,local_values[start_col + REDUCTION_ADDITION_OFFSET + ADDITION_Y_OFFSET + i]);
            yield_constr.constraint_transition(builder, c);
        }
    }
    ...
    for i in 0..24 {
        let mul_tmp1 = builder.mul_extension(bit_selector_val,  local_values[start_col + REDUCTION_ADDITION_OFFSET + ADDITION_CHECK_OFFSET]);
        let sub_tmp1 = builder.sub_extension( local_values[start_col + REDUCE_X_OFFSET + i], local_values[start_col + REDUCTION_ADDITION_OFFSET + ADDITION_SUM_OFFSET + i]);
